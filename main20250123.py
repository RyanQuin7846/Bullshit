# -*- coding: utf-8 -*-
"""
Created on Thu Jan 23 00:17:49 2025
...
# 设置随机种子
seed = 0  # 固定随机种子，确保结果可重复
np.random.seed(seed)

# 定义矩阵的大小和选择点的比例
matrix_size = 34
percentage = 0.01
num_points = int(matrix_size * matrix_size * percentage)

# 生成所有可能的点坐标
all_points = [(i, j) for i in range(matrix_size) for j in range(matrix_size)]

# 随机选择 1% 的点
selected_points = np.random.choice(len(all_points), num_points, replace=False)
selected_coordinates = [all_points[i] for i in selected_points]

refer_point = []
for x, y in selected_coordinates:
    refer_point.append(gt[y,x])
