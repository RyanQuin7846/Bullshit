...
for seed in range(1): 
    ...
    for epoch in range(epochs):
        ...
        grad1 = torch.autograd.grad(torch.norm(cr_loss,p=2), net.parameters(), create_graph=True)
        norm1 = torch.norm(torch.cat([g.view(-1) for g in grad1 if g is not None]))
        grad2 = torch.autograd.grad(mse_collocation, net.parameters(), create_graph=True)
        norm2 = torch.norm(torch.cat([g.view(-1) for g in grad2 if g is not None]))
        target_ratio = (norm1+norm2) / 2
        # if epoch < 1e4:
        #     loss = mse
        # elif epoch < 1e5:
        ...
        # elif epoch<2e5:
            ...
            loss = (target_ratio/norm1) * torch.norm(cr_loss,p=2) + (target_ratio/norm2) * mse_collocation# + 1e3*bc_max_loss + 1e3*bc_min_loss
        # elsE:
        ...
        norm1_list.append((target_ratio/norm1).detach().cpu().numpy())
        norm2_list.append((target_ratio/norm2).detach().cpu().numpy())
        ssim_list.append(s)
