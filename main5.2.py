# -*- coding: utf-8 -*-
"""
Created on Sun Dec 22 22:14:48 2024
...
for seed in range(1): 
    ...
    for epoch in range(epochs):
        ...
        sigma = e.reshape(gt.shape[0],gt.shape[1])
        
        p_g_p_x = torch.autograd.grad(sigma.sum(), x, create_graph=True, retain_graph=True)[0].reshape(gt.shape[0],gt.shape[1])
        p_g_p_y = torch.autograd.grad(sigma.sum(), y, create_graph=True, retain_graph=True)[0].reshape(gt.shape[0],gt.shape[1])
        ...
        cr_loss = (-(torch.reciprocal(sigma)**2)*(p_g_p_x * gradient_x.reshape(gt.shape[0],gt.shape[1])+p_g_p_y * gradient_y.reshape(gt.shape[0],gt.shape[1]))
                        +(torch.reciprocal(sigma) * laplacian.reshape(gt.shape[0],gt.shape[1]))
        ...
        # elif epoch<2e5:
            ...
            loss = torch.norm(cr_loss,p=2)# + 1e3*bc_max_loss + 1e3*bc_min_loss
        # else:
